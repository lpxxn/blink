...
- 与严格 DDD「领域事件」仍有一定距离：事件载荷是手写 struct JSON，尚未与 `domain/event/events.go` 中的另一套命名完全统一（见文末「代码演进」）。
...
发布端实现：`infrastructure/messaging/notification_watermill_publisher.go`。  
消费端解析：`infrastructure/messaging/notification_watermill_consumer.go` 中 `dispatchNotificationEvent`。

**与 `domain/event/events.go` 的关系**：该文件里另有 `reply.posted` 等命名与 `ReplyPostedPayload` 等结构体，**当前 Watermill 路径未使用**；若将来统一，应只保留一套 `type` + 载荷定义，并同步改 Publisher / `dispatchNotificationEvent`，避免两套协议并存。
...
| `domain/event/notification.go` | 当前 Wire 使用的 `type` 字符串常量 |
...
- 协议以 **`type` + JSON 字段** 为准，升级时注意与 `dispatchNotificationEvent` 同步。
- 需要更严格的 DDD 或「恰好一次」，可在本设计之上增量加 **Outbox、幂等键、死信队列** 等，而不必换掉 Watermill 核心用法。
